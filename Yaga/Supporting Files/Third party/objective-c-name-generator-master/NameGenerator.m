...
@property (nonatomic, assign) unsigned int rnd;
@end

@implementation NameGenerator

+ (instancetype)sharedGeneratror {
    static NameGenerator *g = nil;
...
- (id)init
{
	...
	if (self)
	{
        generatedNames = [NSMutableDictionary dictionary];
...
{
    if(self.rnd) {
        return (NSUInteger) floor(self.rnd % max);
    }
...
- (NSString*)nameForPhoneNumber:(NSString*)phoneNumber {
    if(generatedNames[phoneNumber])
        return generatedNames[phoneNumber];
    
    unsigned int seed = [[phoneNumber substringToIndex:7] intValue];
    srandom(seed);
    self.rnd = rand();
    
    NSString *result = @"";
    NSArray *components = [[[self getName] lowercaseString] componentsSeparatedByString:@" "];
    for (NSString *comp in components) {
        if(comp.length > 4)
            result = [result stringByAppendingString:[comp substringToIndex:4]];
        else
            result = [result stringByAppendingString:comp];
        
        if([components lastObject] != comp) {
